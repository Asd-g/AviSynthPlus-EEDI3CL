...
using V_ibool =
    std::conditional_t<std::is_same_v<V_float, Vec4f>, Vec4ib, std::conditional_t<std::is_same_v<V_float, Vec8f>, Vec8ib, Vec16ib>>;
        ...
        if (d->process[plane])
        {
            ...
            for (int y{4 + field_n}; y < paddedHeight - 4; y += 2 * d->vectorSize)
            {
                ...
                for (int vs{0}; vs < d->vectorSize; ++vs)
                {
                    ...
                    memcpy(fpath_all_lines + static_cast<size_t>(line_idx) * dstWidth, fpath_line,
                        static_cast<size_t>(dstWidth) * sizeof(int));
                }
...
template void filterCL_avx2<uint8_t>(const AVS_VideoFrame* src, const AVS_VideoFrame* scp, AVS_VideoFrame* dst, const int field_n,
    bool use_dh, EEDI3CLData* __restrict d, const AVS_FilterInfo* __restrict fi);
template void filterCL_avx2<uint16_t>(const AVS_VideoFrame* src, const AVS_VideoFrame* scp, AVS_VideoFrame* dst, const int field_n,
    bool use_dh, EEDI3CLData* __restrict d, const AVS_FilterInfo* __restrict fi);
template void filterCL_avx2<float>(const AVS_VideoFrame* src, const AVS_VideoFrame* scp, AVS_VideoFrame* dst, const int field_n,
    bool use_dh, EEDI3CLData* __restrict d, const AVS_FilterInfo* __restrict fi);
